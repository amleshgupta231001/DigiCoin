...
//     console.log(`User ${user._id} is requesting to join a game with difficulty "${captchaDifficulty}"`);

//     // Check if user is already in queue
//     for (const difficulty in gameQueue) {
//       if (gameQueue[difficulty].includes(user._id.toString())) {
//         console.warn(`User ${user._id} is already in queue (difficulty: ${difficulty})`);
...
//       if (
//         game.player1 === user._id.toString() || 
//         game.player2 === user._id.toString()
//       ) {
//         console.warn(`User ${user._id} is already in an active game (Game ID: ${gameId})`);
//         return res.status(400).json({ error: 'You are already in a game' });
//       }
//     }

//     // Add user to queue
//     gameQueue[captchaDifficulty].push(user._id.toString());
//     console.log(`User ${user._id} added to queue (difficulty: ${captchaDifficulty})`);

//     // Set timeout for matchmaking (30 seconds)
//     setTimeout(async () => {
//       const index = gameQueue[captchaDifficulty].indexOf(user._id.toString());
//       if (index !== -1) {
//         // Remove from queue after timeout
//         gameQueue[captchaDifficulty].splice(index, 1);
//         console.log(`No opponent found for user ${user._id} within 30 seconds. Creating game against bot...`);
...
//           status: 'completed',
//           captchaDifficulty,
//           winner: user._id, // User wins by default
//         });

//         await game.save();
//         console.log(`Game created against bot. Game ID: ${game._id}`);
...
//           { new: true }
//         );
//         console.log(`Wallet updated for user ${user._id}. New balance: ${walletUpdate.coins}`);
...
//           amount: 100,
//           type: 'credit',
//           description: 'Won against bot',
//           status: 'completed',
//         });
//         await transaction.save();
//         console.log(`Transaction recorded for user ${user._id}. Transaction ID: ${transaction._id}`);
//       }
//     }, 30000);

//     console.log(`Matchmaking started for user ${user._id} with difficulty "${captchaDifficulty}"`);
//     res.json({ message: 'Searching for opponent...', difficulty: captchaDifficulty });
    
//   } catch (error) {
//     console.error(`Error in createGame for user ${req.user?._id}:`, error);
...
//       // Update wallets
//       if (game.winner) {
//         const winnerWallet = await Wallet.findOne({ user: game.winner });
//         const loserWallet = await Wallet.findOne({ 
//           user: game.winner.toString() === game.player1.toString() ? game.player2 : game.player1 
//         });

//         if (winnerWallet) {
//           winnerWallet.coins += 100;
...
//           await Transaction.create({
//             user: loserWallet.user,
//             amount: 50,
//             type: 'debit',
//             description: 'Lost captcha game',
//             status: 'completed',
//           });
//         }
//       }
...
//       isCorrect,
//       nextCaptcha: 'New captcha generated',
//       player1Score: game.player1Score,
//       player2Score: game.player2Score,
//       timeLeft: 60000 - (now - game.startTime),
//     });
//   } catch (error) {
//     res.status(500).json({ error: error.message });
//   }
...
//         timeLeft: 60000 - (new Date() - game.startTime),
//         difficulty: game.captchaDifficulty,
//       });
//     }
...
//       },
//       { $unwind: '$user' },
//       { $project: { _id: 0, userId: '$_id', name: '$user.name', wins: 1 } },
//     ]);
...
//     // Minimum withdrawal amount is 50 rupees
//     if (amount < 50) {
...
//     // Initiate withdrawal
//     const withdrawalResponse = await initiateWithdrawal(user, amount);
